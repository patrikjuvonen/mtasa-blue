...
void CClientPlayerVoice::Init(void)
{
    ...
    unsigned char ucComplexity = m_pVoiceRecorder->GetComputationalComplexity();

    // Setup our BASS playback device
    m_pBassPlaybackStream = BASS_StreamCreate(m_SampleRate / VOICE_SAMPLE_SIZE, m_Channel, BASS_STREAM_AUTOFREE, STREAMPROC_PUSH, NULL);
    ...
    int error;
    m_pOpusDecoderState = opus_decoder_create(m_SampleRate, m_Channel, &error);

    // Initialize our opus decoder
    opus_decoder_ctl(m_pOpusDecoderState, OPUS_SET_COMPLEXITY(&ucComplexity));
...
void CClientPlayerVoice::DecodeAndBuffer(char* pBuffer, unsigned int bytesWritten)
{
    ...
    opus_int16 pTempBuffer[2048];

    opus_decode(m_pOpusDecoderState, reinterpret_cast<unsigned char*>(pBuffer), bytesWritten, pTempBuffer, 120, 0);

    unsigned int uiOpusBlockSize = m_iOpusIncomingFrameSampleCount * m_Channel * VOICE_SAMPLE_SIZE;
    BASS_StreamPutData(m_pBassPlaybackStream, (void*)pTempBuffer, uiOpusBlockSize);
